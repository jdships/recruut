...
const githubButtonVariants = cva(
	"cursor-pointer relative overflow-hidden will-change-transform backface-visibility-hidden transform-gpu transition-transform duration-200 ease-out group whitespace-nowrap focus-visible:outline-hidden inline-flex items-center justify-center whitespace-nowrap font-medium ring-offset-background disabled:pointer-events-none disabled:opacity-60 [&_svg]:shrink-0",
	{
		variants: {
			variant: {
				default:
			...
			size: {
				...
				sm: "h-8 rounded-lg px-3 text-xs gap-1.5 text-xs leading-none [&_svg]:size-3.5 gap-1.5",
...
interface GithubButtonProps
	...
	targetStars?: number;
	/** Whether to show star count even when it's 0 */
	...
	return (
		...
		>
			...
			>
				...
				>
					<span>
						{(currentStars > 0 || showZeroStars) && formatNumber(currentStars)}
					</span>
				</motion.div>
				{fixedWidth && (
